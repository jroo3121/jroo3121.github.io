...
class Game(tk.Tk):
    ...
    def computerMove(self):
        if self.board.isFull() or self.game_over: return None
    ...
    def setupKeyBinds(self):
        self.bind("!", lambda e: (self.ai_level_var.set("Easy"), self.newGame(level=1)))
        self.bind("@", lambda e: (self.ai_level_var.set("Medium"), self.newGame(level=2)))
        self.bind("#", lambda e: (self.ai_level_var.set("Hard"), self.newGame(level=3)))
        self.bind("p", lambda e: (self.multiplayer_var.set(not self.multiplayer_var.get()), self.newGame(multiplayer=self.multiplayer_var.get(), player_starts=self.x_starts_var.get(), level=self.getLevel())))
        self.bind("x", lambda e: (self.x_starts_var.set(not self.x_starts_var.get()), self.newGame(multiplayer=self.multiplayer_var.get(), player_starts=self.x_starts_var.get(), level=self.getLevel())))
        self.bind("n", lambda e: self.newGame(multiplayer=self.multiplayer_var.get(), player_starts=self.x_starts_var.get(), level=self.getLevel()))
        self.bind("r", lambda e: self.newGame(multiplayer=self.multiplayer_var.get(), player_starts=self.x_starts_var.get(), level=self.getLevel()))
        self.bind("q", lambda e: exit())
        cell_keys = {"1": (0,0), "2": (0,1), "3": (0,2), "4": (1,0), "5": (1,1), "6": (1,2), "7": (2,0), "8": (2,1), "9": (2,2)}
        for key, (r, c) in cell_keys.items():
            self.bind(key, lambda e, row=r, col=c: self.cellClick(row, col))
...
class Board(list):
    ...
    def __gt__(self, other):
        return self.amountOccupied() > other.amountOccupied()
...
class AI(object):
    ...
    def AIMove(self, board):
        ...
        if self.lvl == 1: return self.lvlOne(board)
        elif self.lvl == 2: return self.lvlTwo(board)
        elif self.lvl == 3: return self.lvlThree(board)
    ...
    def minimax(self, board, ai_turn):
        if board.xWin(): return -1 
        if board.oWin(): return 1
